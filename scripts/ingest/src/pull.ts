...
function pullLapSamples(
  ...
  const lapEnd = new Date(
    Date.parse(lap.date_start) + lap.lap_duration * 1000,
  ).toISOString();
  ...
  return fetchJson<LocationRow[]>(locationUrl)
    ...
    .then(({ locationRows, carDataRows }) => {
      const durationMs = lap.lap_duration * 1000;
      const clip = <T extends { date: string }>(rows: T[]): T[] =>
        rows
          .map((row) => ({
            ...row,
            t_ms: millisecondsFromLapStart(row.date, lap.date_start!),
          }))
          .filter((row) => row.t_ms >= 0 && row.t_ms <= durationMs)
          .sort((a, b) => a.t_ms - b.t_ms)
          .map(({ t_ms: _tMs, ...row }) => row as T);
...
function mockReplay(): ReplayFile {
  ...
  return {
    ...
    notes: [
      ...
      "Samples are placeholders; real join of car_data/location to lap window is not implemented yet.",
    ],
  };
}
...
async function pullLive(): Promise<ReplayFile> {
  ...
  return {
    ...
    notes: [
      ...
      "Rate-limit / 429 backoff not implemented yet — keep pulls infrequent.",
    ],
  };
}

...
const ASPHALT_PROJECTION_RADIUS_SQUARED =
...
function considerTriangleEdgeXZ(
  x: number,
  z: number,
  startX: number,
  startZ: number,
  ...
  endWeightB: number,
  endWeightC: number,
  out: ClosestTrianglePoint,
): void {
  const edgeX = endX - startX
  const edgeZ = endZ - startZ
  ...
  const alpha =
    lengthSquared > 1e-12
      ? Math.min(
          ...
          Math.max(
            0,
            ((x - startX) * edgeX + (z - startZ) * edgeZ) / lengthSquared,
          ),
        )
      : 0
  const closestX = startX + edgeX * alpha
  const closestZ = startZ + edgeZ * alpha
  const deltaX = x - closestX
  const deltaZ = z - closestZ
  const distanceSquared = deltaX * deltaX + deltaZ * deltaZ
  if (distanceSquared >= out.distanceSquared) return

  out.distanceSquared = distanceSquared
  out.weightA = startWeightA + (endWeightA - startWeightA) * alpha
  out.weightB = startWeightB + (endWeightB - startWeightB) * alpha
  out.weightC = startWeightC + (endWeightC - startWeightC) * alpha
  out.x = closestX
  out.z = closestZ
}
...
function closestPointOnTriangleXZ(
  ...
  out.distanceSquared = Number.POSITIVE_INFINITY
  considerTriangleEdgeXZ(
    x,
    z,
    a.x,
    a.z,
...
export function createAsphaltProjector(
    ...
    for (let offset = 0; offset + 2 < indexCount; offset += 3) {
      ...
      for (
        ...
        for (
          ...
          let row = cells.get(gridX)
          if (!row) {
            row = new Map<number, number[]>()
            cells.set(gridX, row)
          }
          const occupants = row.get(gridZ)
          if (occupants) occupants.push(triangleIndex)
          else row.set(gridZ, [triangleIndex])
        }
      }
    }
  }
  ...
  const closestScratch: ClosestTrianglePoint = {
  ...
  const pointIsInsideTriangleXZ = (
    ...
    return (
      closestPointOnTriangleXZ(x, z, triangle, closestScratch)
        .distanceSquared <= 1e-10
    )
  }
  ...
  return {
    contains(x: number, z: number) {
      ...
      const occupants = cells
        .get(cellCoordinate(x))
        ?.get(cellCoordinate(z))
      if (!occupants) return false
      for (
        let occupantIndex = 0;
        occupantIndex < occupants.length;
        occupantIndex += 1
      ) {
        const triangleIndex = occupants[occupantIndex]
    ...
    project(x: number, z: number) {
      ...
      for (let deltaX = -cellRadius; deltaX <= cellRadius; deltaX += 1) {
        for (let deltaZ = -cellRadius; deltaZ <= cellRadius; deltaZ += 1) {
          ...
          for (
            ...
            const closest = closestPointOnTriangleXZ(
              x,
              z,
              triangle,
              closestScratch,
            ...
            if (
              closest.distanceSquared > ASPHALT_PROJECTION_RADIUS_SQUARED
            ) {
              continue
            }
            const pointY =
            ...
            const isCloser =
              !bestTriangle ||
              closest.distanceSquared < bestDistanceSquared - 1e-9
            const isSamePlaceButHigher =
              bestTriangle !== null &&
              Math.abs(closest.distanceSquared - bestDistanceSquared) <=
                1e-9 &&
              pointY > bestY
            if (isCloser || isSamePlaceButHigher) {
              bestTriangle = triangle
              bestDistanceSquared = closest.distanceSquared
              bestWeightA = closest.weightA
              bestWeightB = closest.weightB
              bestWeightC = closest.weightC
              bestX = closest.x
              bestZ = closest.z
              bestY = pointY
            }
      ...
      return {
        ...
        up: bestTriangle.upA
          .clone()
          .multiplyScalar(bestWeightA)
          .addScaledVector(bestTriangle.upB, bestWeightB)
          .addScaledVector(bestTriangle.upC, bestWeightC)
          .normalize(),
